...
var resultVm = new Vue({
    ...
    created: function () {
        ...
        let userorg=getCookie("userorg");
        let orgtree=getCookie("userorg");
        if(orgtree)orgtree=JSON.parse(getCookie("userorg"));
        if(userid&&userid!=""){
        this.userLoginInfo={
            userid:userid,
            username:username,
    ...
    methods: {
        ...
        fetchTransactions: function (url) {
            ...
            let orgtree=this.userLoginInfo.orgtree;
                if(orgtree&&orgtree[2]){
                    for(let key in orgtree[2]){
                         options.responsibleDeptCode=key;
                    }
                }
