			...
			if ($.isFunction(events)) {
				...
				events(cloneDate(rangeStart), cloneDate(rangeEnd), function(events) {
					...
					for(var i = 0, j = events.length; i < j; i++){
						var event = events[i];
						if(event.isMultiType){
							//isMultiType = true;
							if(event.username && $.inArray(event.username, names) < 0){
								event.index = names.length;
								//names[names.length] = event.username;
							} else{
								event.index = $.inArray(event.username, names);
							}
							//@mattia
							if(event.people){
								var id=event.people.id;
...
function htmlStrongEscape(s) {
	return "<strong>" + s + "<br/></strong>";
}
...
function BasicEventRenderer() {
	...
	var renderDaySegs2 = t.renderDaySegs2;
...
function AgendaView(element, calendar, viewName) {
	...
	function buildSkeleton() {
		...
		if (opt('allDaySlot')) {
			...
			if(isMultiType){
				var menberData = "";
				for(var i = names.length, j = 0; j < i; j++){
					menberData += "<tr style='border: 1px solid #ccc'>" +
						"<th class='" + headerClass + " fc-agenda-axis' style='text-align:center'><strong>" + names[j] + "</strong></th>" +
						"<td>" +
				...
				s = "<table style='width:100%' class='fc-agenda-allday' cellspacing='0'>" +
					"<tr>" +
					"<th class='" + headerClass + " fc-agenda-axis'>" + '全天' + "</th>" +
					"<td>" +
					"<div class='fc-day-content'><div style='position:relative'/></div>" +
					"</td>" +
					"<th class='" + headerClass + " fc-agenda-gutter'>&nbsp;</th>" +
		...
		slotScroller =
			$("<div style='position:absolute;width:100%;overflow-x:hidden;overflow-y:auto'/>")
				.appendTo(slotLayer);
		slotContent =
			$("<div style='position:relative;width:100%;overflow:hidden;display: none'/>")
...
function AgendaEventRenderer() {
	...
	function renderEvents(events, modifiedEventId) {
		...
		if (opt('allDaySlot')) {
			...
			if(isMultiType){
				renderDaySegs2(segmentContainer, compileDaySegs(dayEvents), modifiedEventId);
			} else{
...
function DayEventRenderer() {
	...
	t.renderDaySegs2 = renderDaySegs2;
	...
	function renderDaySegs2(segmentContainer, segs, modifiedEventId) {
		var rowDivs;
		var rowCnt = getRowCnt();
		var colCnt = getColCnt();
		var i = 0;
		...
		for(var i = 0, j = segs.length; i < j; i++){
			if(segs[i].event.index){
				daySegSetTop(segs[i], getRowTops2(segs[i].event.index));
			} else{
				daySegSetTop(segs[i], getRowTops2(0));
			}
		}
	}
	...
	function daySegHTML(segs) { // also sets seg.left and seg.outerWidth
		...
		for (i=0; i<segCnt; i++) {
			...
			html +=
				...
				">" +
				"<div class='fc-event-inner' style='text-align:center;'>";
	...
	function getRowTops2(index) {
		var dayContents = $('.fc-agenda-allday').find('tbody').find('div.fc-day-content > div');
		var tmp = [];
		for(var i = 0, j = dayContents.length; i < j; i ++){
			tmp[i] = dayContents[i].offsetTop;
		}
		var tops = [];
		tops[0] = tmp[index];
		return tops;
	...
	function daySegSetTop(seg, rowTops) { // also triggers eventAfterRender
		//var i;
		//var segCnt = segs.length;
		//var seg;
		var element;
