            ...
            $http.post(url, requestEvents)
             .success(function(response){
                if(response.length == ""){
                    ...
                    if(isMultiType){
                        event.username = event.username && event.username.length > 0 ? event.username : event.people.username;
                        event.index = $.inArray(event.username, names);
                    }
        ...
        editEvent: function (event, isMultiType) {
            ...
            .success(function(response){
                if(true){
                    ...
                    var curEvent = convertEvent(event, isMultiType);
                    if(isMultiType){
                        event.name = event.name && event.name.length > 0 ? event.name : event.people.name;
                        event.index = $.inArray(event.username, names);
                    }
